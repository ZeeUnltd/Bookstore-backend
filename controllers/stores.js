...
module.exports = function (lib) {
            ...
            if (id) {
                lib.db.model('Store')
                    ...
                    .exec(function (err, data) {
                        if (err) return next(controller.RESTError('InternalServerError', err))
                        if (!data) {
                            return next(controller.RESTError('ResourceNotFoundError', 'Store notfound'))
                        }
                        console.log(data)
        ...
        controller.addAction({
                    ...
                    if (data) {
                        ...
                        next(controller.RESTError('InvalidArgumentError', 'No daata received'))
                        controller.addAction({
                            'path': '/stores/{id}',
                            'method': 'PUT',
                            ...
                            var id = req.params.id
                            if (id) {
                                lib.db.model("Store").findOne({
                                    _id: id
                                }).exec(function (err, store) {
                                    if (err) return next(controller.RESTError('InternalServerError', err))
                                    if (!store) return 
                                    next(controller.RESTError('ResourceNotFoundError', 'Storeotfound'))
                                ...
                                next(controller.RESTError('InvalidArgumentError', 'Invalid ideceived'))
                            }
                        })
                        return controller
                    }