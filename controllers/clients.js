...
module.exports = function (lib) {
    ...
    controller.addAction({
        'path': '/clients',
        'method': 'POST',
        'params': [swagger.bodyParam('client', 'The JSON representation of theclient', 'string')],
    ...
    controller.addAction({
        ...
        if (id != null) {
            lib.db.model('Client').findOne({
                _id: id
            }).exec(function (err, client) {
                if (err) return next(controller.RESTError('InternalServerError', err)) if (!client) return next(controller.RESTError('ResourceNotFoundError', 'The client id cannot be found'))
                controller.writeHAL(res, client)
            })
        } else {
            ...
            model.findOne({
                ...
                .exec(function (err, client) {
                    ...
                    client.save(function (err, newClient) {
                        if (err) return next(controller.RESTError('IntrnalServerError', err))
