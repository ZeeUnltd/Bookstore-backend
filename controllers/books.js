...
module.exports = function (lib) {
            ...
            if (id) {
                lib.db.model("Book")
                    ...
                    .exec(function (err, book) {
                        ...
                        if (!book) {
                            return next(controller.RESTError('ResourceNotFoundError',
                                'Book notfound'))
                        }
        ...
        controller.addAction({
                    'path': '/books',
                    'method': 'POST',
                    'params': [ swagger.bodyParam('book', 'JSON representation of the newbook ','string ') ],
                            ...
                            lib.db.model("Book")
                                ...
                                .exec(function (err, bookModel) {
                                    ...
                                    bookModel.save(function (err, book) {
                                        if (err) return next(controller.RESTError('InternalServerError', err)); controller.writeHAL(res, book)
                                    })
                                })
                        } else {
                        ...
                        if(id) {
                          lib.db.model("Book")
                            ...
                            .exec(function(err, book) {
                              if(err) return next(controller.RESTError('InternalServerError', err))
                              if(!book) {
                                return next(controller.RESTError('ResourceNotFoundError', 'Book notfound')) }
